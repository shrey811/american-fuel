...
import { listProductCategory, productCategoryDelete } from 'store/slices/productCategorySlice';
...
const ProductCategoryList = (props: Props) => {
    ...
    const [productCategoryList, listProductCategoryLoading, deleteProductCategoryLoading] = useAppSelector(
        (state) => [
            state.productCategoryReducers.productCategoryList,
            state.productCategoryReducers.listProductCategoryLoading,
            state.productCategoryReducers.deleteProductCategoryLoading
        ],
        shallowEqual
    );
    ...
    // const productCategorySliced = productCategoryList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) || [];
    // console.log({ productCategorySliced });
    ...
    const deleteProductCategory = async () => {
        try {
            const action = await dispatch(productCategoryDelete({ productCategory_id: editId }));
    ...
    const filteredProductCategoryListData = productCategoryList.filter(item =>
    ...
    const sortedData = sortData(filteredproductCategoryListData, getComparator(order, orderBy));

    const csvHeaders = [
        { label: "ProductCategory", key: "Name" }
    ];

    const csvData = productCategoryList.map(item => ({
        Name: item.Name
    }));
    ...
    if (listProductCategoryLoading || deleteProductCategoryLoading) {
    ...
    return (
        <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', padding: '8px' }}>
                    <div className="filterColumn">
                        ...
                        >
                            ...
                            <MenuItem>
                                <CSVLink data={csvData} headers={csvHeaders} filename="product_category_list.csv" style={{ color: '#000', textDecoration: 'none', display: 'flex' }}>
                                    <DownloadIcon />
                                    &nbsp; Download CSV
                                </CSVLink>
                            </MenuItem>
                                ...
                                <MenuItem>
                                    <FormControl variant="outlined" size="small" margin="dense" sx={{ minWidth: 120, marginRight: '16px' }}>
                                        ...
                                        >
                                            ...
                                            <MenuItem value="Name">ProductCategory</MenuItem>
                                        </Select>
                                    </FormControl>
                                </MenuItem>
                                <MenuItem>
                            ...
                            sortedData
                                ...
                                ?.map((item, index) => (
                                    <TableRow key={index} sx={{ background: index % 2 === 0 ? '#FFFFFF' : '#F8F8F8' }}>
                                        ...
                                        {/* //<TableCell>{item.Products.length > 0 ? item.Products[0].Name : "-"}</TableCell> */}
                                        <TableCell className="no-print">
                                            <ListItemIcon sx={tableListItemStyles} onClick={() => {
                                                // Set the edit data with the clicked item
                                                props.setEditProductCategory(item);
            ...
            <TablePagination
                ...
                component="div"
                count={productCategoryList.length}
            ...
            <ConfirmationModal
                ...
                handleConfirmClick={() => deleteProductCategory()}
            />
        </>
    )
}
