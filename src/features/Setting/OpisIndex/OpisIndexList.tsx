...
import { deleteOpisIndex, listOpisIndex } from 'store/slices/opisIndexSlice';
...
const OpisIndexList = (props: Props) => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<{ [key: string]: boolean }>({ ProductCategory: true, Product: true, EffectiveDateTime: true, Rate: true, Unit: true, RateType: true });
  const [order, setOrder] = useState<'asc' | 'desc'>('asc');
  const [orderBy, setOrderBy] = useState<string>('');

  // Fetch data using hooks
  const [opisListData, opisDataLoading] = useAppSelector(
    (state) => [
      state.opisIndexReducers.opisListData,
      state.opisIndexReducers.opisDataLoading,
    ],
    shallowEqual
  );
  ...
  const deleteOPISIndex = async () => {
    try {
      const action = await dispatch(deleteOpisIndex({ ois_id: editId }));
  ...
  const filteredOpisIndexListData = opisListData.filter(item =>
  ...
  const sortedData = sortData(filteredOpisIndexListData, getComparator(order, orderBy));
  ...
  const csvData = opisListData.map(item => ({
    ProductCategory: item.ProductCategory,
    Product: item.Product,
    EffectiveDateTime: item.EffectiveDateTime,
    Rate: item.Rate,
    Unit: item.Unit,
    RateType: item.SubType
  }));
  ...
  const columnOptions = [
    ...
    { name: 'EffectiveDateTime', key: 'EffectiveDateTime' },
    { name: 'Rate', key: 'Rate' },
    { name: 'Unit', key: 'Unit' },
  ...
  return (
    <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', padding: '8px' }}>
            <div className="filterColumn">
              ...
              >
                ...
                <MenuItem>
                  <CSVLink data={csvData} headers={csvHeaders} filename="OPISIndexlist.csv" style={{ color: '#000', textDecoration: 'none', display: 'flex' }}>
                    <DownloadIcon />
                    &nbsp; Download CSV
                  </CSVLink>
                </MenuItem>
                  ...
                  <MenuItem>
                    <FormControl variant="outlined" size="small" margin="dense" sx={{ minWidth: 120, marginRight: '16px' }}>
                      ...
                      >
                        ...
                        <MenuItem value="EffectiveDateTime">EffectiveDateTime</MenuItem>
            ...
            <TableHead>
              <TableRow sx={{ background: '#EFEFEF' }}>
                ...
                {visibleColumns.RateType && (
                  <TableCell sortDirection={orderBy === 'RateType' ? order : false}>
                    <Tooltip title="Sort" placement="top" arrow>
                      <span
                        onClick={() => handleRequestSort('RateType')}
                        style={{ cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                      >
                        RateType
                        {orderBy === 'RateType' ? (
                ...
                sortedData
                  ...
                  .map((item, index) => (
                    <TableRow key={index} sx={{ background: index % 2 === 0 ? '#FFFFFF' : '#F8F8F8' }}>
                      ...
                      {visibleColumns.RateType && <TableCell>{item.SubType}</TableCell>}
      ...
      <ConfirmationModal
        ...
        handleConfirmClick={deleteOPISIndex}
      />
    </>
  );
};

export default OpisIndexList;