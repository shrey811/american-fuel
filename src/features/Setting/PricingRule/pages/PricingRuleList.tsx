...
import { deletePricingRule, listPricingRule } from 'store/slices/pricingRuleSlice';
interface Props {
  setEditData?: any;
  toggleForm?: any;
}

const PricingRuleList = (props: Props) => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<{ [key: string]: boolean }>({ ProductCategory: true, Product: true, Markup: true, Rate: true, Unit: true, RateType: true });
  ...
  const deletePriceRule = async () => {
    try {
      const action = await dispatch(deletePricingRule({ pricing_id: editId }));
  ...
  const csvData = pricingListData.map(item => ({
  ...
  return (
    <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', padding: '8px' }}>
            <div className="filterColumn">
              ...
              >
                ...
                <MenuItem>
                  <CSVLink data={csvData} headers={csvHeaders} filename="PricingRulelist.csv" style={{ color: '#000', textDecoration: 'none', display: 'flex' }}>
                    <DownloadIcon />
                    &nbsp; Download CSV
                  </CSVLink>
                </MenuItem>
      ...
      <ConfirmationModal
        ...
        handleConfirmClick={deletePriceRule}
      />
    </>
  );
};
